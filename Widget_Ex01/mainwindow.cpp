...
#include <QDebug>
#define back_sound ":/sound/Theme.wav"
#define collision_sound ":/sound/collision.wav"
#define cat_sound ":/sound/fazahng.wav"
#define assistance_sound ":/sound/coin.wav"
#define jump_sound ":/sound/jump.wav"
#define bullet_sound ":/sound/fazahng.wav"
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent),
     backgroundMusic(new QSound(back_sound, this))
{
    backgroundMusic->setLoops(QSound::Infinite); // 设置无限循环播放
...
void MainWindow::paintEvent(QPaintEvent*)
{
    ...
    for(int i=0;i<barriers.size();i++){
        ...
        if(barr&&barr->remain(this)&&!barr->touchRole()){
            ...
            // 播放音效
            if (dynamic_cast<Assistance*>(barr)) {
                QSound::play(assistance_sound);
            } else if (dynamic_cast<Wall*>(barr)) {
                QSound::play(collision_sound);
            } else if (dynamic_cast<Cat*>(barr)) {
                QSound::play(cat_sound);
            } else if (dynamic_cast<Bullet*>(barr)) {
                QSound::play(bullet_sound);
            }
            barr->hurt();
